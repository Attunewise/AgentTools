...
- Published links must remain openable after the live source JSONL grows. Preserve the published `indexId` in `sessions/*.ir.jsonl` and restore it while rehydrating so `openLink` does not recompute a different identity from newer source content.
