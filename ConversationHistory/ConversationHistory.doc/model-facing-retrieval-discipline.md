...
MCP search, browse, and status calls default to the current Codex session when the caller omits `session_id` and `index_id`. The MCP resolves that scope by emitting a server-generated response marker and matching the source session JSONL that contains it. Duplicate marker matches are disambiguated through CodexSessionTools thread ancestry when possible and fail closed otherwise. If the MCP cannot prove the current session, it returns an empty scoped result instead of falling back to the shared catalog. All-session search or catalog/status browsing requires the explicit `all_sessions` flag and remains bounded by `start_at`/`start` and `limit`. Retrieval must not index on demand.
